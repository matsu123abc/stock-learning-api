...
【絶対遵守ルール】
- 価格の数値（市場価格・理論価格・その他の価格）を一切生成してはならない
- 価格に関する記述は「高い」「低い」「上回る」「下回る」などの比較表現のみ使用する
- 価格の具体的な数値は絶対に書かない（例：1234.56 など）
- JSON 以外の文章を一切書かない
- コードブロック（```）禁止
...
【価格情報（GPTは数値を使わず比較のみ行うこと）】
市場価格は理論価格より {"高い" if market_price > bs_price_value else "低い"}。
